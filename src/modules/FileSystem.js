...
class Directory {
  ...
  addDirectory(directory) {
    if (this.children[directory.name]) {
      throw new Error(
        `File or directory named ${directory.name} already exists.`,
      );
    }
  ...
  getFile(fileName) {
    if (
      ...
      this.children[fileName] instanceof Directory
    ) {
  ...
  getDirectory(directoryName) {
    if (
      !this.children[directoryName] ||
      this.children[directoryName] instanceof File
    ) {
